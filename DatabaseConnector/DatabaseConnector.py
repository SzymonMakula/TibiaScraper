...
class DatabaseConnector:
        ...
        try:
            while True:
                if not queue.empty():
                    ...
                    char_exists = session.query(
                        exists().where(Characters.name == character_info.name)
                    ).scalar()
                    if not char_exists:
    ...
    def get_polack_nickname(self):
        ...
        random_polack = random.choice(polacks.all())
        return random_polack

    def get_polacks_nicknames(self):
        session = self.load_session()
        polack_nicknames = session.query(Characters.name).filter(
            Characters.nationality == "pl"
        )
        session.close()
    ...
    def get_polacks(self):
        ...
        polack_chars = (
            session.query(
                ...
                Characters.level,
                Characters.vocation,
                Characters.auction_id,
            )
            .filter(Characters.nationality == "pl")
